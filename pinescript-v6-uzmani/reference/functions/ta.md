...
## ta.mjr() - Majority Rule (Özel)

Majority Rule göstergesi, seçilen periyot boyunca fiyatın yükselme/düşüş yüzdesini hesaplar.
...
```pine
//@version=6
indicator("MJR Example")
mjrPeriod = input.int(14, "Period")
upCount = math.sum(close > close[1] ? 1 : 0, mjrPeriod)
MJR = (upCount / mjrPeriod) * 100
plot(MJR, "MJR", color.blue)
hline(70, "Overbought", color.red, hline.style_dashed)
hline(30, "Oversold", color.green, hline.style_dashed)
```

### Doğru MJR vs Yanlış (Stochastic)
Yanlış: `ta.sma(ta.stoch(close, high, low, period), smooth)`
Doğru: `(Yükselen bar sayısı / Periyot) * 100`
