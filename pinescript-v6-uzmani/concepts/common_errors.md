...
### TP/SL Sistemi - Kısmi Kapanış Takibi Hatası

**Sorun:** TP1 ve TP2 ile kısmi kapanış yaparken, kapanan miktar doğru takip edilmiyor. TP1 kapandıktan sonra kalan miktar hesaplanamıyor ve TP2 yanlış çalışıyor.

**Çözüm:** Her TP tetiklendiğinde kapanan miktarı ayrı değişkenlerde takip edin:

```pine
// TP Trigger bayrakları ve miktar takibi
var bool tp1Triggered = false
var bool tp2Triggered = false
var float tp1ClosedQty = 0.0  // TP1'de kapatılan miktar
var float tp2ClosedQty = 0.0  // TP2'de kapatılan miktar

// Pozisyon yokken sıfırla
if noPos
    tp1Triggered := false
    tp2Triggered := false
    tp1ClosedQty := 0.0
...
// TP1 tetiklendiğinde miktarı güncelle
if longTP1Hit
    tp1Triggered := true
    tp1ClosedQty := math.min(tp1CloseQty, currentLongQty)

// TP2 kontrolü - kalan miktarı hesapla
remainingAfterTP1 = currentLongQty - tp1ClosedQty
qtyToCloseTP2 = math.max(tp2CloseQty, remainingAfterTP1)
```

**Önemli:** Her zaman kalan miktarı hesaplayın ve o miktarı kullanarak kapatın.

---

### TP/SL Çalışmama Sorunu ve Çözümü (Piko Projesi Deneyimi)

**Sorun:** TP (Take Profit) seviyesine fiyat geldiğinde kapanış yapmıyor.

**Çözüm 1 - strategy.exit kullanımı:**
```pine
strategy(
     "Strateji",
...
// TP için strategy.exit + limit
if inLong and useTP1
    strategy.exit(id="TP1", from_entry="Long", qty=1, limit=tp1Price, comment="TP1")

// SL için strategy.exit + stop
if inLong and useSL
    strategy.exit(id="SL", from_entry="Long", qty=posQty, stop=slPrice, comment="SL")
```

**Önemli:**- `calc_on_every_tick = true`: Intra-bar hesaplama- `process_orders_on_close = true`: Bar kapanışında emir işleme- `strategy.exit` + `limit` = Fiyat seviyeye değdiğinde otomatik kapanış

**Çözüm 2 - Reverse sorunu:**
Aynı bar'da hem TP hem Reverse çalışmıyor. Reverse geldiğinde TP emirlerini iptal et:

```pine
if inLong
    if shortEntry  // Reverse sinyali
        strategy.cancel("TP1_Long")
        strategy.cancel("TP2_Long")
        strategy.cancel("SL_Long")
        strategy.close("Long")
        strategy.entry("Short", qty=posQty)
    else
        // TP/SL normal çalışır
        if useTP1
            strategy.exit(id="TP1_Long", from_entry="Long", qty=tp1CloseQty, limit=longTP1Price)
```
